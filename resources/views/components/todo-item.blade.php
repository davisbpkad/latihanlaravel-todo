<li class="flex items-center justify-between bg-blue-50 hover:bg-blue-100 transition p-4 rounded-lg shadow group">
    ...
    <form action="{{ route('todos.destroy', $todo) }}" method="POST">
        @csrf
        @method('DELETE')
        <button class="justify-center py-3 px-6 border-2 border-black text-sm font-bold rounded-lg text-white bg-red-400 hover:bg-red-700 hover:text-white
        shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] opacity-0 group-hover:opacity-100
        focus:outline-none transition-all duration-200 active:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] active:translate-x-[3px] active:translate-y-[3px]">Hapus</button>
    </form>
</li> 