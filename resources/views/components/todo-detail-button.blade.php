...
<form method="GET" class="inline-block">
    <input type="hidden" name="show" value="{{ $todo->id }}">
    <input type="hidden" name="page" value="{{ request('page', 1) }}">
    <button type="submit"
        {{ $attributes->merge([
            'class' => 'justify-center py-1 px-3 border-1 border-black text-sm font-bold rounded-lg text-white bg-yellow-400 hover:bg-yellow-700 
